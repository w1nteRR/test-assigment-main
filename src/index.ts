...
import { SendNotificationSpecificPersonController } from "./core/presentation/controllers/send-notification-specific-person";
import { SendNotificationSpecificPersonUseCase } from "./core/domain/usecases/send-notification-specific-person.usecase";
import { AdaptiveCardsRepository } from "./core/infrastructure/repositories/messages.repository";
...
const adaptiveCardsRepository = new AdaptiveCardsRepository(notificationApp);

const sendNotificationSpecificPersonUseCase =
  new SendNotificationSpecificPersonUseCase(adaptiveCardsRepository);

const sendNotificationSpecificPersonController =
  new SendNotificationSpecificPersonController(
    sendNotificationSpecificPersonUseCase
  );

expressApp.post("/api/notification", async (req, res) =>
  sendNotificationSpecificPersonController.handle(req, res)
);

expressApp.post("/api/null", async (req, res) => {
